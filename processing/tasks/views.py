...
def trip_updates_json(request):
    file_path = settings.BASE_DIR / "tasks" / "files" / "trip_updates.json"
    return FileResponse(open(file_path, "rb"), filename="trip_updates.json")


@xframe_options_exempt
def trip_updates_pb(request):
    file_path = settings.BASE_DIR / "tasks" / "files" / "trip_updates.pb"
    return FileResponse(
