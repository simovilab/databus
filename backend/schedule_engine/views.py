...
def schedule(request):
    file_path = settings.BASE_DIR / "schedule_engine" / "files" / "bUCR_GTFS.zip"
...
def vehicle_pb(request):
    file_path = settings.BASE_DIR / "schedule_engine" / "files" / "vehicle_positions.pb"
    return FileResponse(
...
def trip_updates_json(request):
    file_path = settings.BASE_DIR / "schedule_engine" / "files" / "trip_updates.json"
    return FileResponse(open(file_path, "rb"), filename="trip_updates.json")


@xframe_options_exempt
def trip_updates_pb(request):
    file_path = settings.BASE_DIR / "schedule_engine" / "files" / "trip_updates.pb"
    return FileResponse(
